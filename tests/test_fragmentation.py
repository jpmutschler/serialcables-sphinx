...
class TestPacketSequence:
    ...
    def test_expect_next(self):
        seq = PacketSequence()
        assert seq.expect_next(0) == True
        assert seq.expect_next(1) == True
        assert seq.expect_next(3) == False  # Out of order
        assert seq.expect_next(2) == True  # Still expecting 2
...
class TestFragmentationDetection:
    ...
    def test_small_payload_no_fragmentation(self):
        ...
        assert builder.needs_fragmentation(payload) == False
    ...
    def test_just_over_limit(self):
        ...
        assert builder.needs_fragmentation(payload) == True
...
class TestBuildFragmented:
    ...
    def test_single_fragment_message(self):
        ...
        assert result.is_fragmented == False
        assert result.fragments[0].is_first == True
        assert result.fragments[0].is_last == True
    ...
    def test_multi_fragment_message(self):
        ...
        assert result.is_fragmented == True
        
        # Check first fragment
        assert result.fragments[0].is_first == True
        assert result.fragments[0].is_last == False
        assert result.fragments[0].sequence == 0
        
        # Check middle fragment
        assert result.fragments[1].is_first == False
        assert result.fragments[1].is_last == False
        assert result.fragments[1].sequence == 1
        
        # Check last fragment
        assert result.fragments[2].is_first == False
        assert result.fragments[2].is_last == True
...
class TestFragmentationConfig:
    ...
    def test_default_values(self):
        ...
        assert config.inter_fragment_delay_ms == FragmentationConstants.DEFAULT_INTER_FRAGMENT_DELAY_MS
