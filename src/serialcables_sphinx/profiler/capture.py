...
class DeviceProfiler:
        ...
        for attempt in range(self._config.retry_count + 1):
            try:
                ...
                cmd = CapturedCommand(
                    ...
                    data_type=data_type,
                    data_type_name=NVMeDataStructureType(data_type).name if data_type is not None else None,
                    config_id=config_id,
        ...
        if ctrl_result.success:
            ...
            for ctrl_id in controller_ids:
                self._capture_command(
                    ...
                    request_func=lambda cid=ctrl_id: self._sphinx.nvme_mi.controller_health_status(cid, eid=self._eid),
    ...
    def _capture_data_structure_commands(self) -> None:
        ...
        if result and result.success:
            ...
            self._profile.metadata.nvme_mi_major_version = result.decoded_fields.get('NVMe-MI Major Version')
            self._profile.metadata.nvme_mi_minor_version = result.decoded_fields.get('NVMe-MI Minor Version')
        ...
        for port_id in range(self._profile.metadata.num_ports):
            self._capture_command(
                ...
                category=CommandCategory.DATA_STRUCTURE.value,
                opcode=NVMeMIOpcode.READ_NVME_MI_DATA_STRUCTURE,
                request_func=lambda pid=port_id: self._sphinx.nvme_mi.get_port_info(port_id=pid, eid=self._eid),
    ...
    def _capture_configuration_commands(self) -> None:
        ...
        for config_id, name in config_ids:
            self._capture_command(
                ...
                request_func=lambda cid=config_id: self._sphinx.nvme_mi.configuration_get(cid, eid=self._eid),
    ...
    def _capture_vpd_commands(self) -> None:
        ...
        while offset < self._config.vpd_max_offset:
            result = self._capture_command(
                ...
                request_data=[offset & 0xFF, (offset >> 8) & 0xFF, chunk_size & 0xFF, (chunk_size >> 8) & 0xFF],
            )
