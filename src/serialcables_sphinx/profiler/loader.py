...
from serialcables_sphinx.profiler.profile import DeviceProfile, CapturedCommand
...
class ProfileLoader:
    ...
    def _extract_vpd(cls, state: "MockDeviceState", vpd_commands: List[CapturedCommand]) -> None:
        ...
        sorted_cmds = sorted(
            ...
            key=lambda c: (c.request_data[0] | (c.request_data[1] << 8)) if len(c.request_data) >= 2 else 0
        )
