...
class MockTransport(FragmentedTransportMixin):
    ...
    def __init__(
        self,
        state: Optional[MockDeviceState] = None,
        verbose: bool = False,
    ):
        """
    ...
    def _handle_health_status(self, data: bytes) -> bytes:
        ...
        ccs = (
            (int(s.ready) << 0) |
            (int(s.fatal_status) << 1) |
            ((s.shutdown_status & 0x03) << 2)
        )
