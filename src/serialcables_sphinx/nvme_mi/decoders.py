    ...
    def decode(self, data: bytes, response: DecodedResponse) -> DecodedResponse:
        if len(data) < 1:
            response.decode_errors.append(
                f"Health status response too short: {len(data)} bytes"
            )
            return response
        ...
        self._add_field(
            response, "Ready (RDY)", rdy, data[0:1],
            description="Controller ready to process commands" if rdy else "Controller not ready"
        )
        self._add_field(
            response, "Controller Fatal Status (CFS)", cfs, data[0:1],
            description="Fatal controller condition detected" if cfs else "No fatal condition"
        )
        self._add_field(
            response, "Shutdown Status (SHST)", str(ShutdownStatus(shst)), data[0:1]
        )
        self._add_field(
            response, "NVM Subsystem Reset Occurred", nssro, data[0:1]
        )
        self._add_field(
            response, "Controller Enable Change Occurred", ceco, data[0:1]
        )
        self._add_field(
            response, "Namespace Attribute Changed", nace, data[0:1]
        )
        ...
        if len(data) > 1:
            ...
            self._add_field(
                response, "Critical Warning", f"0x{data[1]:02X}", data[1:2],
                description=str(cw)
            )
        ...
        if len(data) > 4:
            ...
            self._add_field(
                response, "Drive Life Used", life_str, data[4:5],
                description=desc
            )
        ...
        if len(data) > 5:
            spare_thresh = data[5]
            self._add_field(
                response, "Available Spare Threshold",
        ...
        if len(data) > 6:
            ...
            self._add_field(
                response, "Available Spare", spare_str, data[6:7],
                description=desc
            )
...
class ControllerHealthStatusDecoder(ResponseDecoder):
    ...
    def decode(self, data: bytes, response: DecodedResponse) -> DecodedResponse:
        ...
        self._add_field(
            response, "Shutdown Status", str(ShutdownStatus(shst)), data[2:3]
        )
        ...
        if len(data) >= 12:
            ...
            thresh = data[11]
            self._add_field(
                response, "Available Spare",
                self._decode_percentage(spare),
                data[10:11]
            )
            self._add_field(
                response, "Spare Threshold",
                self._decode_percentage(thresh),
                data[11:12]
            )
        ...
        if len(data) > 12:
            ...
            if pct_used != 255 and pct_used >= 100:
                desc = "Exceeded rated endurance"
            self._add_field(
                response, "Percentage Used", pct_str, data[12:13],
                description=desc
            )
...
class SubsystemInfoDecoder(ResponseDecoder):
    ...
    def decode(self, data: bytes, response: DecodedResponse) -> DecodedResponse:
        if len(data) < 4:
            response.decode_errors.append(
                f"Subsystem info too short: {len(data)} bytes"
            )
            return response
        ...
        if len(data) >= 8:
            ...
            self._add_field(
                response, "Optional Commands Supported",
    ...
    def decode(self, data: bytes, response: DecodedResponse) -> DecodedResponse:
        ...
        for i in range(num_entries):
            if offset + 2 > len(data):
                response.decode_errors.append(
                    f"Truncated controller list at entry {i}"
                )
                break
        ...
        self._add_field(
            response, "Controller IDs", controllers, data[2:offset]
        )
...
class ConfigurationGetDecoder(ResponseDecoder):
    ...
    def decode(self, data: bytes, response: DecodedResponse) -> DecodedResponse:
        ...
        self._add_field(
            ...
            data.hex(),
            data,
            description="Raw configuration data"
        )
...
class GenericDataStructureDecoder(ResponseDecoder):
    ...
    def decode(self, data: bytes, response: DecodedResponse) -> DecodedResponse:
        self._add_field(
            ...
            data.hex(),
            data,
            description="Raw data structure bytes"
        )
        return response
