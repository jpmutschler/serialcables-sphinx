...
from serialcables_sphinx.nvme_mi.constants import NVMeDataStructureType, ConfigurationIdentifier
...
class NVMeMIClient:
    ...
    def _send_mi_command(
        ...
        return self._decoder.decode_mctp_response(
            mctp_payload=bytes([parsed.msg_type | (0x80 if parsed.integrity_check else 0)]) + parsed.payload,
    ...
    def health_status_poll(self, eid: Optional[int] = None) -> DecodedResponse:
        """
    ...
    def read_data_structure(
        ...
        data_type_val = data_type.value if isinstance(data_type, NVMeDataStructureType) else data_type
        return self._send_mi_command(eid, request, data_type=data_type_val)
    
    def get_subsystem_info(self, eid: Optional[int] = None) -> DecodedResponse:
        """
    ...
    def get_controller_list(self, eid: Optional[int] = None) -> DecodedResponse:
        """
    ...
    def mi_reset(self, eid: Optional[int] = None) -> DecodedResponse:
        """
    ...
    def discover_subsystem(self, eid: Optional[int] = None) -> dict:
        """
