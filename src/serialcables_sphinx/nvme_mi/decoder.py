...
class NVMeMIDecoder:
    ...
    def decode_response(
        ...
        try:
            opcode_enum = NVMeMIOpcode(opcode_val)
        except ValueError:
            opcode_enum = opcode_val  # Keep as int for vendor-specific
        ...
        if len(raw_data) < 4:
            ...
            try:
                status = NVMeMIStatus(status_val)
            except ValueError:
                status = status_val  # Keep raw value for unknown status
        ...
        if decoder is not None:
            ...
            response.decode_errors.append(
                f"No decoder registered for opcode 0x{opcode_val:02X}"
            )
    ...
    def decode_mctp_response(
        ...
        if msg_type != 0x04:  # NVMe-MI message type
            ...
            response.decode_errors.append(
                f"Not an NVMe-MI message: type=0x{msg_type:02X}"
            )
            return response
