...
def cmd_compare(args) -> int:
    ...
    if only_in_1:
        ...
        for opcode, dt, cfg in sorted(only_in_1):
            print(f"  Opcode 0x{opcode:02X}" + (f" DataType={dt}" if dt else "") + (f" Config={cfg}" if cfg else ""))
    ...
    print(f"  Profile 1: min={profile1.metadata.min_latency_ms:.1f}, "
          f"max={profile1.metadata.max_latency_ms:.1f}, "
          f"avg={profile1.metadata.avg_latency_ms:.1f}")
    print(f"  Profile 2: min={profile2.metadata.min_latency_ms:.1f}, "
...
Examples:
  ...
  %(prog)s --mock-test samsung_990.json
    ...
    skip_group.add_argument("--skip-data-struct", action="store_true", help="Skip data structure commands")
    skip_group.add_argument("--skip-config", action="store_true", help="Skip configuration commands")
    skip_group.add_argument("--skip-vpd", action="store_true", help="Skip VPD read")
    skip_group.add_argument("--skip-admin", action="store_true", help="Skip admin tunneled commands")
    
    # VPD options
    vpd_group = parser.add_argument_group("VPD Options")
    vpd_group.add_argument("--vpd-max", type=int, default=4096, help="Max VPD offset to read (default: 4096)")
    vpd_group.add_argument("--vpd-chunk", type=int, default=256, help="VPD chunk size (default: 256)")
    
    # Output options
    output_group = parser.add_argument_group("Output Options")
    output_group.add_argument("--summary", action="store_true", help="Show profile summary (with --load)")
    output_group.add_argument("--verify", action="store_true", help="Verify profile checksum (with --load)")
    output_group.add_argument("--json", dest="json_output", action="store_true", help="Output as JSON (with --load)")
    output_group.add_argument("--compact", action="store_true", help="Compact JSON output (no indentation)")
    output_group.add_argument("-q", "--quiet", action="store_true", help="Quiet mode (minimal output)")
    output_group.add_argument("--debug", action="store_true", help="Show debug information on errors")
